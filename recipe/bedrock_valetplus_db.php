 ...
 *
 * Assumes that Bedrock runs locally on a Valet+ environment:
...
task('pull:db', function () use ($getLocalEnv, $getRemoteEnv, $urlToDomain) {
    ...
    runLocally("cd {{local_root}} && wp search-replace '{$remoteUrl}' '{$localUrl}' --skip-themes --url='{$remoteDomain}' --network");
    // Also replace domain (multisite WP also uses domains without protocol in DB)
    runLocally("cd {{local_root}} && wp search-replace '{$remoteDomain}' '{$localDomain}' --skip-themes --url='{$remoteDomain}' --network");
...
task('push:db', function () use ($getLocalEnv, $getRemoteEnv, $urlToDomain) {
    ...
    writeln("<comment>Exporting Valet+ DB to {$exportAbsFile}</comment>");
